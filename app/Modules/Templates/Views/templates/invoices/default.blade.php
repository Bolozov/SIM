...
<body>
    ...
    <table style="width: 50%; float: right">
        <tbody>
            <tr>
                <td colspan="6" class="amount">{{ mb_strtoupper(trans('bt.subtotal')) }}</td>
                <td class="amount">{{ $invoice->amount->formatted_subtotal }}</td>
            </tr>
            <tr>
                <td colspan="6" class="amount">{{ mb_strtoupper('timbre fiscal') }}</td>
                <td class="amount">{{ number_format($invoice->companyProfile->timbre_fiscal , 3) }} DT</td>
            </tr>

            @if ($invoice->discount > 0)
            <tr>
                <td colspan="6" class="amount">{{ mb_strtoupper(trans('bt.discount')) }}</td>
            ...
            <tr>
                <td colspan="6" class="amount">{{ mb_strtoupper($tax->name) }} ({{ $tax->percent }})</td>
                <td class="amount">{{ $tax->total }}</td>
            </tr>
            @endforeach

            <tr>
                <td colspan="6" class="amount">{{ mb_strtoupper(trans('bt.total')) }}</td>
                <td class="amount">{{ $invoice->amount->formatted_total }}</td>
            </tr>
            <tr>
                <td colspan="6" class="amount">{{ mb_strtoupper(trans('bt.paid')) }}</td>
                <td class="amount">{{ $invoice->amount->formatted_paid }}</td>
            </tr>
            <tr>
                <td colspan="6" class="amount">{{ mb_strtoupper(trans('bt.balance')) }}</td>
